...
class NKLandscape:
    ...
    def _construct_sequential_dependency_matrix(self):
        ...
        if self.K > 0:
            for offset in range(self.K):
                dependency_matrix += np.eye(self.N, k=offset + 1) + np.eye(
                    self.N, k=-(self.N - (offset + 1))
                )
    ...
    def _check_dependency_matrix(self):
        if self.dependency_matrix.shape != (self.N, self.N):
            raise ValueError(
                "The dependency matrix must be of shape (N, N) = ({}, {})".format(
                    self.N, self.N
                )
            )

        if not np.array_equal(
            self.dependency_matrix, self.dependency_matrix.astype(bool)
        ):
        ...
        if (K == 0) and (self.dependency_matrix.sum() != self.N):
            raise ValueError(
                "When K = 0 the dependency matrix can only be the identity matrix"
            )

        if not np.array_equal(
            self.dependency_matrix.sum(axis=1), np.full((1, self.N), self.K + 1)[0]
        ):
            raise ValueError(
                "The rows of the dependency matrix must each sum up to K={}".format(
                    self.K
                )
            )
    ...
    def calculate_fitness(self, solution):
        ...
        for i in range(self.N):
            ...
            rolled_dependency_indicies = np.roll(
                dependency_indicies, -np.argwhere(dependency_indicies == i)[0]
            )
